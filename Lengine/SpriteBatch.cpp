...
namespace Lengine {
	...
	Glyph::Glyph(const glm::vec4 &recpos, const glm::vec4& recuv
		...
		for (int x = 0;x < 2;x++) {
			for (int y = 0;y < 2;y++) {
				...
				//printf("vertices[%d][%d] uvRec(x,y) = (%f,%f)\n", x, y, vertices[x][y].uv.u, vertices[x][y].uv.v);
			}
		}
	...
	void SpriteBatch::draw(const glm::vec4 &recpos, const glm::vec4& recuv, const GLuint& textureID
		...
		glm::vec2 norDirect;
		if (recpos.z > 0.0f&&recpos.w > 0.0f) {
			if (direct.x == 0 && direct.y == 0)
			{
				_glyphs.push_back(Glyph(recpos, recuv, textureID, depth, color));
			}
			else
			{
			...
			printf("Invalid (width, height) = (%f, %f)\n", recpos.z, recpos.w);
		}
	}
